...
Status: proposed architecture; shared backend is not yet implemented or deployed.
...
Keep the Flutter website on Vercel. Deploy one shared API on Cloud Run and use
Firestore for tenant registrations, onboarding state, employee credentials and
sessions. Retain each company's existing Sheets/Drive data for the initial migration.
...
[Firestore server access and IAM](https://docs.cloud.google.com/firestore/native/docs/security/iam).
...
- Authorize every employee and owner operation against current tenant membership
  ...
  never grants authority. Firestore server IAM does not replace these checks.
...
Suggested API contract (not currently available):
...
The deployment provider/project and canonical API/app origins must be selected
before implementing and validating the complete hosted flow. Setting a dummy
gateway URL or suppressing the current warning does not enable SaaS login.
