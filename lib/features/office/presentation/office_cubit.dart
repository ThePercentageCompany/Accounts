...
class OfficeCubit extends Cubit<OfficeState> {
  ...
  OfficeCubit(this.repository) : super(const OfficeState());

  Future<bool> run([String? action, Map<String, dynamic>? payload]) async {
    if (state.busy) return false;
    emit(state.copyWith(busy: true, error: null));
    try {
      if (action != null) await repository.command(action, payload);
      final data = OfficeData.fromJson(await repository.command('officeLoad'));
      emit(OfficeState(data: data));
      return true;
    } catch (e) {
  ...
  Future<bool> runBatch(List<MapEntry<String, Map<String, dynamic>?>> commands) async {
    if (state.busy) return false;
    emit(state.copyWith(busy: true, error: null));
    try {
      for (final cmd in commands) {
        await repository.command(cmd.key, cmd.value);
      }
      final data = OfficeData.fromJson(await repository.command('officeLoad'));
      emit(OfficeState(data: data));
      return true;
