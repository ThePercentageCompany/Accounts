...
class GoogleApi {
  ...
  Future<Map<String,dynamic>> command(String action,[Map<String,dynamic>? data]) async {
    ...
    if(response.statusCode==401||response.statusCode==403){session.expire('Google access is no longer authorized. Please sign in again.');throw StateError('Google session expired. Reconnect Google.');}
